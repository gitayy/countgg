...
export const SnackbarComponent = () => {
  ...
  useEffect(() => {
    socket.on(`achievement`, function (data) {
      setSnack({
        message: `Achievement unlocked: ${data.name}`,
        severity: 'success',
        open: true,
        url: `/counter/${counter?.username}`,
      })
      setConfetti(true)
      setTimeout(() => {
        setSnack({
    ...
    socket.on(`item`, function (data) {
      setSnack({
        message: `New ${data.item ? data.item.category : 'item'} unlocked: ${data.item ? data.item.name : '???'}`,
        severity: 'success',
        open: true,
        url: `/prefs`,
      })
    ...
    <>
      {snack && (
        <Snackbar
          sx={{ cursor: 'pointer' }}
          open={snack.open}
          onClick={() => navigate(snack.url)}
          anchorOrigin={{ vertical: 'top', horizontal: 'center' }}
        >
          <Alert severity={(snack.severity as AlertColor) || 'success'}>{snack.message}</Alert>
        </Snackbar>
      )}
      {confetti && (
        <Confetti
          run={confetti}
          recycle={false}
          height={height || Math.floor(window.innerHeight)}
          width={width || Math.floor(window.innerWidth)}
        />
      )}
      {/* <Confetti /> */}
