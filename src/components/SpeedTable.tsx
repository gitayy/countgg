  ...
  ({
    ...
    const handleChangePage = (_event, newPage) => {
      ...
      if (onEnsurePageLoaded) {
        onEnsurePageLoaded(
          newPage,
          selectedCounters.map((counter) => counter.uuid),
        )
      }
    }

    const handleCounterSelection = (selectedCounterNames: string[]) => {
      const countersFromUsername = Object.values(cachedCounters).filter((counter) => selectedCounterNames.includes(counter.username))
        ...
        {isSplitTable && (
          <TableCell>
            ...
            {canModerateSplits && onToggleSplitFake && (
              ...
              >
                {pendingSplitKeys.includes(splitKey(row.obj.start, row.obj.end))
                  ? 'Saving...'
                  : row.obj.isFake
                    ? 'Mark real'
                    : 'Mark fake'}
                            ...
                            <Box key={dist.uuid}>
                              <Box
                                sx={{
                                  ...
                                  left: `${pct(dist.min)}%`,
                                  top: y,
                                  width: `${Math.max(1, pct(dist.plotMax) - pct(dist.min))}%`,
                                  height: 2,
                                  bgcolor: color,
                                }}
                              />
                              ...
                              <MuiTooltip
                                title={`P75: ${formatClockTime(dist.plotMax)} (Full max: ${formatClockTime(dist.max)}, chart max: ${formatClockTime(globalMax, 3)})`}
                              >
                                <Box
                                  sx={{
                                    position: 'absolute',
                                    left: `${pct(dist.plotMax)}%`,
                                    top: y - 7,
                                    width: 2,
                                    height: 16,
                                    bgcolor: color,
