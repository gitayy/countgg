  ...
  return (
    <>
      ...
      <Typography sx={{ mb: 1 }} variant="body2">
        {dailyRobs && dailyRobs.length} {dailyRobs && dailyRobs.length != 1 ? 'robs' : 'rob'}
      </Typography>
                            ...
                            avatar={
                              ...
                              cachedCounters[rob.counterUUID].name && (
                                <Avatar
                                  ...
                                  alt={`${cachedCounters[rob.counterUUID].name}`}
                                  src={`${(cachedCounters[rob.counterUUID].avatar.length > 5 && `https://cdn.discordapp.com/avatars/${cachedCounters[rob.counterUUID].discordId}/${cachedCounters[rob.counterUUID].avatar}`) || `https://cdn.discordapp.com/embed/avatars/0.png`}`}
                                ></Avatar>
                              )
                            }
                            title={
                              <Link
                                color={cachedCounters[rob.counterUUID].color}
                                underline="hover"
                                href={`/counter/${cachedCounters[rob.counterUUID].username}`}
                                onClick={(e) => {
                                  e.preventDefault()
                                  navigate(`/counter/${cachedCounters[rob.counterUUID].username}`)
                                }}
                              >
                                {cachedCounters[rob.counterUUID].name}
                              </Link>
