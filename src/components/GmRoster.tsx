...
export function GmRoster(props) {
  ...
  for(const counter in props.unapproved) {
      statRows.push({discordID: props.unapproved[counter].discordId, discordCreation: snowflakeToTime(props.unapproved[counter].discordId).toLocaleString(), username: props.unapproved[counter].username, nickname: props.unapproved[counter].name, id: props.unapproved[counter]['uuid'], numericID: props.unapproved[counter].id, roles: props.unapproved[counter]['roles'] })
  }
  const fa_rows: GridRowsProp = statRows;

return (
  <Container maxWidth="xl" sx={{ bgcolor: 'primary.light', p: 2}}>
    <DataGrid rows={fa_rows} columns={fa_columns} autoHeight={true} hideFooter={false} disableColumnMenu={false} checkboxSelection onSelectionModelChange={(newSelection) => {
