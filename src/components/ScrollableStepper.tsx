    ...
    <Box overflow="auto">
      <Stepper orientation="vertical" activeStep={activeStep}>
        {steps.map((step, index) => {
          const reward = items
            ? items.filter((item) => {
                return item.levelToUnlock === index + 1
              })
            : []
          ...
          return (
            <Step key={index} onClick={() => handleStepClick(index)}>
              <StepLabel
                optional={
                  index === activeStep ? <Typography>XP Needed: {levelThresholds[index].minXP.toLocaleString()} </Typography> : null
                }
                sx={{
                ...
                {counter && (
                  <Box sx={{ width: '50%' }}>
                    <LinearProgressWithLabel
                      color={hasReachedLevel ? 'success' : 'secondary'}
                      progress={counter.xp}
                      max={levelThresholds[index].minXP}
                    />
                  </Box>
                )}
              </StepLabel>
              {index === activeStep ? (
                <>
                  {counter &&
                    (isItemUnlocked ? (
                      <Typography>
                        Item unlocked: {reward[0].name} ({reward[0].category})
                      </Typography>
                    ) : level > index ? (
                      <Typography>Reward available! Press the gift box to receive your reward!</Typography>
                    ) : (
                      <Typography>
                        You're just {(levelThresholds[index].minXP - counter.xp).toLocaleString()} xp away from unlocking this reward!
                      </Typography>
                    ))}
                  {counter &&
                    (isItemUnlocked ? (
                      <ItemBox item={reward[0]} />
                    ) : level > index ? (
                      <GiftBox level={activeStep + 1} />
                    ) : (
                      <Typography variant="body2" sx={{ fontSize: 12 }}>
                        1 reward at LVL {activeStep + 1}
                      </Typography>
                    ))}
                </>
              ) : null}
