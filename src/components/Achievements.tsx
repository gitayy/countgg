...
export function Achievements({ achievements, locked, counterAchievements, counter }: Props) {
  ...
  const { totalCounters } = useContext(UserContext)

  console.log(achievements)
  console.log(counterAchievements)
  const achievementList = achievements.map((achievement) => {
    ...
    return (
      <Grid
        ...
        lg={4}
        xl={3}
        sx={{ p: 1 /*filter: locked ? 'grayscale(100%)' : '', opacity: locked ? '0.75' : '1'*/ }}
      >
        <Link
          color={'inherit'}
          underline="none"
  ...
  return (
    <>
      <Grid container>{achievementList}</Grid>
    </>
  )
}
