      ...
      <Box sx={{ display: 'flex', flexDirection: 'row', width: '100%', background: `${theme.palette[color].main}30` }}>
        <Box sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', flexGrow: 1, p: 2 }}>
          <CardContent>
            ...
            {counterAchievement &&
              !counterAchievement.isComplete &&
              counterAchievement.progress !== undefined &&
              achievement.maxProgress !== undefined && (
                <LinearProgressWithLabel color="success" progress={counterAchievement.progress} max={achievement.maxProgress} />
              )}
          ...
          <CardActions sx={{ display: 'flex', justifyContent: 'flex-end' }}>
            <Typography variant="body2" color="text.secondary" sx={{ display: 'flex', flexDirection: 'column' }}>
              {ofall
                ? `${achievement.countersEarned} / ${ofall} (${Math.round((achievement.countersEarned / ofall) * 10000) / 100}%)`
                : achievement.countersEarned}
