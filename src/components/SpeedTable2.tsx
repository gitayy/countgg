  ...
  if (!speed || !thread) {
    ...
    const leaderboardRows = speed.map((obj, index) => (
      <TableRow key={index}>
        ...
        <TableCell>
          <Link
            underline="hover"
            href={`/thread/${thread.name}/${obj.start_uuid}`}
            onClick={(e) => {
              e.preventDefault()
              navigate(`/thread/${thread.name}/${obj.start_uuid}`)
            }}
          >
            Yep
          </Link>
        ...
        <TableCell>
          <Link
            underline="hover"
            href={`/thread/${thread.name}/${obj.end_uuid}`}
            onClick={(e) => {
              e.preventDefault()
              navigate(`/thread/${thread.name}/${obj.end_uuid}`)
            }}
          >
            Umm
          </Link>
    ...
    const pbLeaderboardRows = pbLeaderboard.map((row, index) => (
      <TableRow key={row.counter}>
        ...
        <TableCell>
          <Link
            underline="hover"
            href={`/thread/${thread.name}/${row.obj.start_uuid}`}
            onClick={(e) => {
              e.preventDefault()
              navigate(`/thread/${thread.name}/${row.obj.start_uuid}`)
            }}
          >
            Yep
          </Link>
        ...
        <TableCell>
          <Link
            underline="hover"
            href={`/thread/${thread.name}/${row.obj.end_uuid}`}
            onClick={(e) => {
              e.preventDefault()
              navigate(`/thread/${thread.name}/${row.obj.end_uuid}`)
            }}
          >
            Umm
          </Link>
    ...
    return (
      <TableContainer>
        ...
        <Typography sx={{ mt: 2, mb: 2 }} variant="body2">
          {sumCounts.toLocaleString()} {sumCounts != 1 ? 'splits' : 'split'}
        </Typography>
        <Table>
          <TableHead>
            <TableRow>
