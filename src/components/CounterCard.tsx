                ...
                <Box sx={{ p: props.boxPadding }} className={`border_${props.counter.cardBorderStyle}-parent`}>
                  <CardMedia
                    ...
                    sx={{
                      ...
                      backgroundImage:
                        props.counter.avatar.length > 5
                          ? `url(https://cdn.discordapp.com/avatars/${props.counter.discordId}/${props.counter.avatar})`
                          : `url(${CggLogo2})`,
                    }}
                  />
                </Box>
                  ...
                  <CardContent sx={{ flex: '1 0 auto', p: 0, '&:last-child': { pb: 0 } }}>
                    ...
                    <Typography component="div" variant="h6" color={'text.primary'} sx={{ display: 'flex', alignItems: 'center' }}>
                      <Chip
                        component={'span'}
                        size="small"
                        title={`${props.counter.xp} / ${xpToNext}`}
                        label={level}
                        color={level === 50 ? 'warning' : level > 40 ? 'success' : level > 20 ? 'secondary' : 'primary'}
                        sx={{}}
                      ></Chip>
                      &nbsp;
                      {props.counter.emoji
                        ? `${props.counter.emoji} ${props.counter.name} ${props.counter.emoji}`
                        : props.counter.name}
                      &nbsp;
                      <Typography component="span" variant="body2" sx={{}}>
                        @{props.counter.username}
                      </Typography>
                    </Typography>
                    <Typography className={'title-default'} variant="subtitle1" color={'text.secondary'} component="div">
                      {props.counter.roles.includes('admin') ? (
                        <Chip component={'span'} size="small" label={'Admin'} sx={{ mr: 1, bgcolor: 'gold', color: 'black' }} />
                      ) : props.counter.roles.includes('mod') ? (
                        <Chip
                          component={'span'}
                          size="small"
                          label={'Moderator'}
