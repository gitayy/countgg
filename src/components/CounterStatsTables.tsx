...
export function CounterStatsTables(props) {
    function buildStandardTableRows(props) {
        ...
        for(const season in props.stats) {
            allRows.push(<tr key={`${season}`}>
                <td>{season != 'Career' && 'Season'} {season}</td>
    ...
    function buildMultipleTables(props) {
        ...
        for(const split in props.splits) {
            allRows.push(<div key={`${props.splits[split]}`}>
...
return (<>
    ...
    <Box sx={{bgcolor: 'background.paper'}}>
        <DataGrid rows={rows} columns={columns} autoHeight={true} hideFooter={true} disableColumnMenu={true} components={{ Toolbar: GridToolbar }} componentsProps={{toolbar: {printOptions: {disableToolbarButton:true}}}}></DataGrid>
    </Box>
    {/* </div> */}
    </>
)
