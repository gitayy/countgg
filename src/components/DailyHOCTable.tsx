...
export const DailyHOCTable = ({ dailyHOC, name, countName, mini }: Props) => {
  ...
  if (dailyHOC !== undefined && dailyHOC !== null) {
    const rows = Object.entries(dailyHOC)
      .map(([authorUUID, { counter, counts }]) => ({
        counter,
        counts,
      }))
      ...
      <>
        ...
        {!mini && (
          <Typography sx={{ mb: 1 }} variant="body2">
            {sumCounts} {sumCounts != 1 ? 'counts' : 'count'}
          </Typography>
        )}
                        ...
                        avatar={
                          ...
                          row.counter.name && (
                            <Avatar
                              ...
                              alt={`${row.counter.name}`}
                              src={`${(row.counter.avatar.length > 5 && `https://cdn.discordapp.com/avatars/${row.counter.discordId}/${row.counter.avatar}`) || `https://cdn.discordapp.com/embed/avatars/0.png`}`}
                            ></Avatar>
                          )
                        }
                        title={
                          <Link
                            color={'inherit'}
                            underline="hover"
                            href={`/counter/${row.counter.username}`}
                            onClick={(e) => {
                              e.preventDefault()
                              navigate(`/counter/${row.counter.username}`)
                            }}
                          >
                            {row.counter.name}
                          </Link>
