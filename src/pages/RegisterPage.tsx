...
export const RegisterPage = () => {
  ...
  const [counter_pronouns, setPronouns] = useState<number>(
    (areArraysEqual(counter?.pronouns || [], pronouns[0]) && 0) ||
      (areArraysEqual(counter?.pronouns || [], pronouns[1]) && 1) ||
      (areArraysEqual(counter?.pronouns || [], pronouns[2]) && 2) ||
      (!counter?.pronouns[0] && 2) ||
      0,
  )
  ...
  const namesNext = async () => {
    ...
    //   }
      ...
      try {
        ...
        setSnackbarOpen(true)
        setSnackbarMessage(
          'Error: Submission rejected by server. If this comes as a surprise, please reach out to discord mods via DM!',
        )
      }
    }
  }
          ...
          <ThemeProvider theme={lightTheme}>
            <Box sx={{ bgcolor: 'background.paper' }}>
              <Count
                ...
                key={`fakeCount_${Math.random()}`}
                thread={{}}
                socket={{}}
                post={fakePost({ ...counter, name: name, username: username, color: color })}
                counter={counter}
                maxWidth={'32px'}
                maxHeight={'32px'}
              />
          ...
          {step == 1 && (
            <>
              ...
              <Paper sx={{ p: 2 }}>
                ...
                <Typography>
                  We recommend you do not use any self-identifying information like your IRL name when creating your profile.
                </Typography>
                <TextField
                  sx={{ m: 2 }}
                  id="name"
                  onInput={(e) => {
                    ...
                    counter.name = (e.target as HTMLInputElement).value
                  }}
                  label="Display Name"
                  defaultValue={name}
                ></TextField>
                <TextField
                  ...
                  onInput={(e) => {
                    ...
                    counter.username = (e.target as HTMLInputElement).value
                  }}
                  label="Username"
                ...
                <FormControl sx={{ m: 2 }}>
                  ...
                  >
                    {pronouns.map((pos, posindex) => {
                      return (
                        <MenuItem
                          key={`pronoun_${posindex}`}
                          value={posindex}
                        >{`${pos[0]}, ${pos[1]}, ${pos[2]}, ${pos[3]}`}</MenuItem>
                      )
                    })}
                  </Select>
                </FormControl>
                ...
                <TextField
                  ...
                  onInput={(e) => {
                    ...
                    counter.color = (e.target as HTMLInputElement).value
                  }}
                  label="Color"
