...
export const TheRockPage = () => {
    ...
    const handlePress = () => {
      ...
      if(now > lastThrottle.current + 100) {
        ...
        socket.emit('rock')
        setPresses(prevPresses => {return prevPresses + 1;})
        setOkScale(prevScale => {
          if(prevScale + 1 === 1101) {reverse.current = true;} else if(prevScale - 1 === 99) {reverse.current = false;}
          if(reverse.current) {return prevScale - 1} else {return prevScale + 1;}
        })
      }
    }

return (
    <Box sx={{ display: 'flex', alignItems: 'center', flexDirection: "column", justifyContent: 'center', bgcolor: 'background.paper', color: 'text.primary', flexGrow: 1, p: 2, background: 'url("https://i.imgur.com/ByxzxU4.jpg")', backgroundRepeat: 'no-repeat', backgroundSize: '100% 100%',}}>
      {presses > 0 && rockCount > 0 && <Typography sx={{position: "absolute", bottom: 0, zIndex: 999}}>{rockCount.toLocaleString()}</Typography> }
