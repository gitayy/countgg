    ...
    return (
      <Box sx={{ bgcolor: 'primary.light', flexGrow: 1, p: 2 }}>
        <>
          ...
          <Button onClick={makeCounter} sx={{ m: 1, p: 1 }} variant="contained">
            Make Counter
          </Button>
          <Button onClick={makeDiscordVerified} sx={{ m: 1, p: 1 }} variant="contained">
            Make Discord Verified
          </Button>
          <Button onClick={deny} sx={{ m: 1, p: 1 }} variant="contained">
            Deny
          </Button>
        </>
        {adminSelectionModel.map((val) => (
          <h1>UUID {val}</h1>
        ))}
      </Box>
    )
