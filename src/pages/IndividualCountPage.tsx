            ...
            <Stack direction="column" alignItems="center" sx={{ width: '50%' }}>
              <Typography sx={{ mb: 2 }} variant="h5" color={'text.primary'}>
                <Link
                  onClick={(e) => {
                    e.preventDefault()
                    navigate(`/thread/${thread_name}`)
                  }}
                  color={'text.primary'}
                  underline="hover"
                  href={`/thread/${thread_name}`}
                >
                  {thread.title}
                </Link>
              </Typography>
              <Typography sx={{ mb: 2 }} variant="h6" color={'text.secondary'}>
                This is one update from the thread. Click the thread name above to see the latest counts, or see the context of this
                post{' '}
                <Link
                  onClick={(e) => {
                    e.preventDefault()
                    navigate(`/thread/${thread_name}?context=${specificCount[0].uuid}`)
                  }}
                  underline="always"
                  color={'text.secondary'}
                  href={`/thread/${thread_name}?context=${specificCount[0].uuid}`}
                >
                  here.
                </Link>
              </Typography>
            ...
            <Stack direction="column" alignItems="center" sx={{ width: '100%' }}>
              ...
              <Typography sx={{ mb: 2 }} variant="h6" color={'text.secondary'}>
                ...
                <Link
                  onClick={(e) => {
                    e.preventDefault()
                    navigate(`..?context=${specificCount[0].uuid}`)
                  }}
                  underline="always"
                  color={'text.secondary'}
                  href={`..?context=${specificCount[0].uuid}`}
                >
                  here.
                </Link>
              </Typography>
