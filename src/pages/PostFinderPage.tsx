...
import { Container, Box, FormControl, InputLabel, Select, MenuItem, Button, Input, Alert, AlertColor, Snackbar, FormControlLabel, Checkbox, SelectChangeEvent, Typography, Stack, Link, useMediaQuery, Theme, Toolbar, Tooltip, Pagination } from '@mui/material';
      ...
      if(rawCount) {
        try {
          if(rawCount.length > 0) {
              setButtonDisable(true);
                const res = await findPostByThreadAndRawCount(rawCount, selectedThread ? selectedThread.uuid : undefined).then(({ data }) => {
                  for (const counter of data.counters) {
                    addCounterToCache(counter);
                  }
                  setLoadedPosts(data.posts);
    ...
    const findComment = async () => {
      if(comment) {
        try {
          if(comment.length > 0) {
              setButtonDisable(true);
                const res = await findPostByThreadAndComment(comment, selectedThread ? selectedThread.uuid : undefined, ).then(({ data }) => {
                  for (const counter of data.counters) {
                    addCounterToCache(counter);
                  }
                  setLoadedPosts(data.posts);
    ...
    if(counter && !allThreadsLoading && allThreads) {
        ...
        {uuid.length >= 0 && <>
            <Box sx={{bgcolor: 'background.paper', color: 'text.primary', p: 3}}> 
                  <Select
                value={selectedThread ? selectedThread.uuid : 'all'}
                onChange={handleThreadSelection}
                sx={{mr: 2}}
              >
                <MenuItem key={'all'} value={'all'}>All Threads</MenuItem>
              ...
              <Button variant='contained' disabled={buttonDisable} onClick={findCountNumber}>Submit</Button>
            ...
            <Box sx={{ bgcolor: 'background.paper', display: 'flex', justifyContent: 'center', flexGrow: 1, p: 2 }}>
              {isDesktop ? (
                ...
                <Stack direction="column" alignItems="center" sx={{width: '50%'}}>
                    ...
                    <Pagination count={Math.ceil(loadedPosts.length / 100)} page={page}  
                    ...
                    {loadedPosts.slice(page*100 - 100, page*100).map((count, countIndex) => {
                      ...
                      const associatedThread = loadedThreads && loadedThreads.length > 0 && loadedThreads.filter(thread => thread.uuid === count.thread)[0] || fakeThread();
                    return (<>
                      <Typography sx={{mb: 2}} variant="h6" color={"text.secondary"}><Link onClick={(e) => {e.preventDefault();navigate(`/thread/${associatedThread.name}`);}} underline='always' color={"text.secondary"} href={`/thread/${associatedThread.name}`}>{associatedThread.title}</Link></Typography>
                      <Typography sx={{mb: 2}} variant="h6" color={"text.secondary"}><Link onClick={(e) => {e.preventDefault();navigate(`/thread/${associatedThread}?context=${count.uuid}`);}} underline='always' color={"text.secondary"} href={`/thread/${associatedThread.name}?context=${count.uuid}`}>Context</Link></Typography>
                        <Count key={count.uuid} post={count} thread={associatedThread} renderedCounter={cachedCounters[count.authorUUID]} maxWidth={'32px'} maxHeight={'32px'} />
                        </>)
                      })}
                      <Pagination count={Math.ceil(loadedPosts.length / 100)} page={page}  
                        onChange={handlePageChange} /> 
