...
const BlogCreatePage = () => {
  ...
  const handlePosting = async () => {
    if (inputRef.current) {
      try {
        const res = await saveBlog(title, inputRef.current.value, tags)
        console.log(res)
        if (res.status == 201) {
          console.log('yes')
      ...
      {counter && !loading ? (
        <>
          ...
          <Typography variant="body2">
            {inputRef.current ? inputRef.current.value.length : 0} / {(20000).toLocaleString()}
          </Typography>
          <TextField label="Add a tag" variant="outlined" fullWidth inputRef={tagInputRef} onKeyDown={handleTagAdd} />
          <Box mt={2}>
            {tags.map((tag, index) => (
              <Chip key={index} label={tag} onDelete={() => handleTagDelete(tag)} style={{ margin: '4px' }} />
            ))}
          </Box>
