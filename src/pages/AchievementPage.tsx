  ...
  if (!loading && achievement && !achievementLoading && isMounted.current) {
    const counter_achievement =
      counterAchievements && counter
        ? counterAchievements.find((counterachievement) => {
    ...
    return (
      <Box sx={{ bgcolor: 'primary.light', flexGrow: 1, p: 2 }}>
        ...
        <AchievementSmall
          ...
          locked={
            false /*!achievements.some(userAchievement => userAchievement.achievementId === achievement.id && userAchievement.isComplete)*/
          }
        ></AchievementSmall>
        {countersWhoEarnedAchievement && countersWhoEarnedAchievement.length > 0 && (
          <>
            Earned by&nbsp;
            {countersWhoEarnedAchievement.map((counter, index) => {
              if (index <= 199) {
                return (
                  <Typography component={'span'}>
                    ...
                    {index + 1 < countersWhoEarnedAchievement.length ? ', ' : ''}
                  </Typography>
                )
              } else if (index == 200) {
