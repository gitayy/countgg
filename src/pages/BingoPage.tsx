...
export const BingoPage = () => {
  ...
  useEffect(() => {
    ...
    const handleList = (data: { activeGames: BingoGameListItem[]; myActiveGameId?: number | null }) => {
  ...
  const metaPills = (game: BingoGameListItem, keyPrefix: string) =>
    [
      ...
      {
        label: (game.mode || 'bingo').toUpperCase(),
        icon: <ExtensionRoundedIcon sx={{ fontSize: 15, color: intelIconColor }} />,
        hint: 'Mode',
      },
      {
        label: getBingoRankLabel(game.rank).toUpperCase(),
        icon: <SportsEsportsRoundedIcon sx={{ fontSize: 15, color: intelIconColor }} />,
        hint: 'Rank',
      },
  ...
  return (
    ...
    >
      ...
      <Grid container spacing={2} sx={{ flex: 1, height: '100%', overflow: 'hidden' }}>
        ...
        <Grid item xs={12} md={12} lg={8} sx={{ height: { lg: '100%' }, overflowY: { lg: 'auto' }, order: { xs: 1, lg: 1 } }}>
          {myActiveGameId ? (
            <Paper
              sx={{
            ...
            >
              <BingoGameView gameId={myActiveGameId} embedded onLeave={() => setMyActiveGameId(null)} />
            </Paper>
          ) : (
            <Paper
              sx={{
            ...
            >
              <Box sx={{ textAlign: 'center' }}>
                <Typography variant="body1" sx={{ color: mutedTextColor }}>
                  {!counter || !counter.roles?.includes('counter') ? 'Only counters can create games.' : ''}
                </Typography>
              </Box>
              <Button
                variant="contained"
        ...
        <Grid item xs={12} md={12} lg={4} sx={{ height: { lg: '100%' }, overflowY: 'auto', order: { xs: 2, lg: 2 } }}>
          <Paper
            sx={{
              border: `1px solid ${panelBorderColor}`,
          ...
          >
            ...
            {rightTab === 0 && (
              <Box sx={{ p: 2 }}>
                {otherActiveGames.length === 0 && (
                  <Typography variant="body1" sx={{ color: mutedTextColor }}>
                    No other active lobbies right now.
                  </Typography>
                )}
                <Stack spacing={1.5}>
                      ...
                      <Box sx={{ minWidth: 0 }}>
                        <Stack direction="row" alignItems="center" spacing={1} sx={{ mb: 0.6 }}>
                          ...
                          <Chip
                            ...
                            sx={{
                              bgcolor:
                                game.status === 'running' ? `${theme.palette.success.main}22` : `${theme.palette.warning.main}2a`,
                              color: game.status === 'running' ? theme.palette.success.main : theme.palette.warning.main,
            ...
            {rightTab === 1 && (
              <Box sx={{ p: 2 }}>
                {completedLoading && (
                  <Typography variant="body2" sx={{ color: mutedTextColor, mb: 1 }}>
                    Loading...
                  </Typography>
                )}
      ...
      >
        ...
        <DialogContent>
          <TextField
            ...
            onKeyDown={(e) => {
              if (e.key === 'Enter' && joinCodeInput.trim()) handleJoinWithCode()
            }}
        ...
        <DialogActions>
          ...
          <Button variant="contained" disabled={!joinCodeInput.trim()} onClick={handleJoinWithCode}>
            Join
          </Button>
        </DialogActions>
      </Dialog>
