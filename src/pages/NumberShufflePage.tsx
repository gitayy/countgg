      ...
      <Grid
        ...
        sx={{
          ...
          opacity:
            numbers[index] !== 0
              ? `1`
              : !isAnimationPlaying && findPossibleIndicesForNextMove(numbers, currentNumber).includes(index)
                ? `1`
                : `0.25`,
        }}
      >
          ...
          <Grid
            ...
            sx={{
              ...
              background:
                numbers[index] !== 0
                  ? `${rainbowColors[index]}`
                  : findPossibleIndicesForNextMove(numbers, currentNumber).includes(index)
                    ? `${rainbowColors[index]}`
                    : `${rainbowColors[index]}`,
            }}
          >
            ...
            sx={{
              ...
              background:
                numbers[index] !== 0
                  ? `${rainbowColors[index]}`
                  : findPossibleIndicesForNextMove(numbers, currentNumber).includes(index)
                    ? `${rainbowColorsWithOpacity[index]}`
                    : `${rainbowColors[index]}`,
    ...
    <Box sx={{ bgcolor: 'primary.light', flexGrow: 1, display: 'flex', color: 'text.primary' }}>
      <Grid container spacing={0} sx={{ flexGrow: 1 }}>
        <Grid item xs={6} sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            ...
            <Grid container sx={modalStyle}>
                ...
                {counter ? (
                  <Box>
                    <Typography
                      ...
                      align="center"
                      m={1}
                      onClick={() => startGame(false, undefined, highScoreSeed)}
                      sx={{ cursor: 'pointer' }}
                    >
              ...
              <Grid item xs={12} lg={6}>
                <Button
                  ...
                  color="primary"
                  size="small"
                  style={{ marginTop: '1vh', width: '95%', height: '5vh', marginLeft: '10px' }}
                >
              ...
              <Grid item xs={12}>
                {scoreData && (
                  <ErrorBoundary comment="Bar Graph">
                    <MUIBarGraph scores={scoreData} />
                  </ErrorBoundary>
                )}
              </Grid>
            </Grid>
              ...
              <Grid item xs={12} lg={12}>
                <TableContainer component={Paper}>
                  <Table>
                    ...
                    <TableBody>
                      {gameHistory.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((game, index) => (
                        <TableRow key={index}>
                          ...
                          <TableCell>
                            <IconButton
                              color="primary"
                              onClick={() => {
                                startGame(false, undefined, game.seed)
                                setHistoryDialogOpen(false)
                              }}
                            >
          ...
          >
            {currentNumber > 0 ? (
              <NumberAnimator number={currentNumberCopy} startRange={1} endRange={1000} animationDuration={animationDuration} />
            ) : gameStatus === 'won' ? (
              'W'
            ) : (
              ''
          ...
          {gameStatus !== 'Loading...' && gameStatus === 'in_progress' && (
            <Button
              ...
              sx={{ width: '95%', height: '5vh', mb: 2, flexGrow: 1, fontSize: '2.5rem' }}
            >
              Shuffle
            </Button>
          )}
