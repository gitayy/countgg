...
export const AdminThreadPage = () => {
  ...
  const sendValues = async () => {
    if (name) {
      try {
        const res = await adminCreateThread(
          name,
          title,
          description,
          rules,
        ...
        <Container maxWidth="xl" sx={{ bgcolor: 'primary.light', flexGrow: 1, p: 2 }}>
          ...
          <Box sx={{ bgcolor: 'white', color: 'black', p: 3 }}>
            ...
            <FormControl variant="standard" sx={{}}>
              ...
              <Input onInput={(e) => setTitle((e.target as HTMLInputElement).value)} defaultValue={title} value={title} id="title" />
            </FormControl>
            <FormControl variant="standard" sx={{}}>
            ...
            <FormControlLabel control={<Checkbox checked={locked} onChange={(e) => setLocked(e.target.checked)} />} label="Locked" />
            <FormControlLabel
              control={<Checkbox checked={autoValidated} onChange={(e) => setAutoValidated(e.target.checked)} />}
              label="autoValidated"
            />
            <FormControlLabel
              control={<Checkbox checked={resetOnMistakes} onChange={(e) => setResetOnMistakes(e.target.checked)} />}
              label="Reset on Mistakes"
            />
            <FormControlLabel
              control={<Checkbox checked={allowDoublePosts} onChange={(e) => setAllowDoublePosts(e.target.checked)} />}
