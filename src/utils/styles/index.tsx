...
export function LinearProgressWithLabel(
  props: LinearProgressProps & { color: string; progress: number; max: number; dontUseComplete?: boolean } = {
    color: 'primary',
    progress: 0,
    max: 100,
    dontUseComplete: false,
  },
) {
    ...
    <Box sx={{ display: 'flex', alignItems: 'center', filter: 'none' }}>
      ...
      <Box sx={{ minWidth: 35 }}>
        <Typography variant="body2" color="text.secondary">
          {normalise >= 100
            ? !props.dontUseComplete
              ? `Complete`
              : `${props.progress.toLocaleString()}/${props.max.toLocaleString()}`
            : `${props.progress.toLocaleString()}/${props.max.toLocaleString()}`}
        </Typography>
